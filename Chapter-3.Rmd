...
str_c("https://www.theguardian.com/world/coronavirus-outbreak+uk/uk?page=", 1:2) %>% 
  lapply(function(URL) {
  page <- read_html(URL)
  title <- html_text(html_nodes(page, ".js-headline-text"))
  time <- html_text(html_nodes(page, ".fc-date-headline"))[1]
  data.frame(time, title)
  }) %>% 
  reduce(rbind) %>% 
...
#%>% tm_map(stemDocument)
  ...
  ggwordcloud(words_frequency$Words, words_frequency$Freq, max.words = 100)
...
words <- left_join(unnest_tokens(data_cleansed, words, title), AFINN, by=c("words"="word"))
