...
```{r rpart2, fig.height=4}
dat %>% 
  rpart::rpart(formula = died ~ ., cp = .003) %>% # TODO tune cp
  rattle::fancyRpartPlot(palettes = 'OrRd',
                         sub = 'cp = .003', main = '1. döntési fa') 
```
...
dat %>% 
  mutate(
    date = as.numeric(difftime(date, lubridate::dmy("01/01/2020"), units = 'days'))
  )
```
